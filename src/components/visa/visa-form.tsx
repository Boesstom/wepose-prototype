...
export function VisaForm({ visaId }: VisaFormProps) {
    ...
    const addDocumentCondition = (docId: string) => {
        setDocuments(prev => prev.map(d => {
            if (d.id === docId) {
                const currentConditions = d.config?.conditions || [];
                return {
                    ...d,
                    config: {
                        ...d.config,
                        conditions: [
                            ...currentConditions,
                            { field: 'jobStatus', operator: 'in', value: '' }
                        ],
                        isConditional: true
                    }
                };
    ...
    const removeDocumentCondition = (docId: string, idx: number) => {
        setDocuments(prev => prev.map(d => {
            if (d.id === docId && d.config?.conditions) {
                const newConditions = d.config.conditions.filter((_, i) => i !== idx);
                return {
                    ...d,
                    config: {
                        ...d.config,
                        conditions: newConditions,
                        isConditional: newConditions.length > 0
                    }
                };
            }
            return d;
    ...
    const updateDocumentCondition = (docId: string, idx: number, key: keyof DocumentCondition, value: any) => {
        setDocuments(prev => prev.map(d => {
            if (d.id === docId && d.config?.conditions) {
                const newConditions = [...d.config.conditions];
                            ...
                            {configuringDocId && (
                                <div className="space-y-4 py-2">
                                    ...
                                    <div className="border-t pt-4 mt-4 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <div className="space-y-0.5">
                                                <Label>Conditional Requirement</Label>
                                                <p className="text-xs text-muted-foreground">Is this document only required under specific circumstances?</p>
                                            </div>
                                            <Switch
                                                checked={documents.find(d => d.id === configuringDocId)?.config?.isConditional || false}
                                                onCheckedChange={c => {
                                                    updateDocumentConfig(configuringDocId, 'isConditional', c);
                                                    if (c && !(documents.find(d => d.id === configuringDocId)?.config?.conditions?.length)) {
                                                        addDocumentCondition(configuringDocId);
                                                    }
                                                }}
                                            />
                                        </div>

                                        {documents.find(d => d.id === configuringDocId)?.config?.isConditional && (
                                            <div className="space-y-3 p-3 bg-muted/30 rounded-md border">
                                                <p className="text-sm font-medium">Document Conditions</p>
                                                {documents.find(d => d.id === configuringDocId)?.config?.conditions?.map((cond, idx) => (
                                                    <div key={idx} className="flex items-center gap-2">
                                                        <Select value={cond.field} onValueChange={v => updateDocumentCondition(configuringDocId, idx, 'field', v as any)}>
                                                            ...
                                                            <SelectContent>
                                                                ...
                                                                <SelectItem value="travelHistory">Travel History</SelectItem>
                                                                <SelectItem value="otherVisa">Other Visa</SelectItem>
                                                            </SelectContent>
                                                        </Select>

                                                        <Select value={cond.operator} onValueChange={v => updateDocumentCondition(configuringDocId, idx, 'operator', v as any)}>
                                                            <SelectTrigger className="w-[140px]"><SelectValue /></SelectTrigger>
                                                            <SelectContent>
                                                        ...
                                                        <Input
                                                            className="flex-1"
                                                            placeholder="Value (comma separated for list)"
                                                            value={cond.value || ''}
                                                            onChange={e => updateDocumentCondition(configuringDocId, idx, 'value', e.target.value)}
                                                        />

                                                        <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive" onClick={() => removeDocumentCondition(configuringDocId, idx)}>
                                                            <HugeiconsIcon icon={Delete02Icon} className="size-4" />
                                                        </Button>
                                                    </div>
                                                ))}
                                                <Button variant="outline" size="sm" onClick={() => addDocumentCondition(configuringDocId)} className="mt-2 text-xs">
                                                    <HugeiconsIcon icon={PlusSignIcon} className="size-3 mr-1" /> Add Condition
                                                </Button>
                                            </div>
                                        )}
                                    </div>
