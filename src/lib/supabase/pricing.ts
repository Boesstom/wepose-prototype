...
import { Visa, PricingCategory } from '@/types/visa';
...
export async function updateVisaPrice(id: string, updates: { price?: number; price_agent?: number, pricing_breakdown?: any[] }) {
  const supabase = createClient();
...
export async function bulkUpdatePrices(
  ...
  // For increase/decrease/set we need to fetch current values for math and JSON mutating
  const { data: visas } = await supabase
    .from('visas')
    .select(`id, price, price_agent, pricing_breakdown`)
    .in('id', visaIds);

  if (!visas) return;

  for (const v of visas) {
    if (target === 'price' || target === 'price_agent') {
      let currentVal = v[target] || 0;
